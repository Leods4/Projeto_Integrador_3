...
class Aluno extends Pessoa {
    ...
    public function buscarDetalhes(): ?array {
        if (!$this->id) {
            throw new RuntimeException("ID do aluno não definido.");
        }

        $query = "SELECT curso, fase, total_horas FROM alunos WHERE id = :id LIMIT 1";
        $stmt = $this->db->prepare($query);
